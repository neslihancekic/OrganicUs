...
class ProductListing extends Component {
    ...
    constructor (props) {
        ...
        this.state = { limit: 5, hasMoreItems: true, products: []};
    ...
    fetchMoreItems = async () => {
        if (this.state.limit >= this.props.colSize.length) {
            this.setState({ hasMoreItems: false });
            return;
        }
        
        await listProduct(this.state.limit)
        .then(data => {
            if(data.err){ 
                        ...
                        {products.length > 0 ?
                            ...
                            <div className="row">
                                <div className="col-sm-12 text-center section-b-space mt-5 no-found" >
                                    ...
                                    <Link to={`${process.env.PUBLIC_URL}/home`} className="btn btn-solid">continue shopping</Link>
                                </div>
                            </div>
                        }
                    </div>
...
const mapStateToProps = (state, ownProps) => ({
    products: getVisibleproducts(ownProps.products, state.filters),
